...
class UnifiedBiometricFetcher:
    def __init__(self, port=4370, timeout=1.0, password=0, no_push=False):
        ...
        self.shift_sync_cache = {}  # Fallback cache for Shift Type sync timestamps
        self.device_registry = {}   # Primary: devices registered in ERPNext, keyed by serial_number
        ...
        # Load device registry from ERPNext — replaces SHIFT_DEVICE_MAP in config.env
        self.device_registry = self._load_device_registry()

    def _load_device_registry(self):
        """
        Fetches all Active Biometric Device Registration records from ERPNext.
        Returns a dict keyed by serial_number for O(1) lookup during sync.
        Falls back to empty dict if ERPNext is unreachable (tool still works via SHIFT_DEVICE_MAP).
        """
        site = self.config.get('FRAPPE_SITE')
        if not site or not self.config.get('API_KEY'):
            return {}
        try:
            url = f"{site.rstrip('/')}/api/method/tb_hotel_core.tb_hotel_core.api.biometric.get_active_devices"
            response = self.session.get(url, timeout=10)
            if response.status_code == 200:
                devices = response.json().get("message", [])
                registry = {}
                for d in devices:
                    sn = d.get("serial_number") or d.get("name")
                    if sn:
                        registry[sn] = d
                print(f"[*] Loaded {len(registry)} device(s) from ERPNext registry.")
                return registry
            else:
                print(f"[!] ERPNext device registry returned HTTP {response.status_code}. Using config.env fallback.")
        except Exception as e:
            print(f"[!] Could not load ERPNext device registry: {e}. Using config.env fallback.")
        return {}

    def update_device_sync_status(self, serial_number, timestamp, error=None):
        """
        Updates last_sync and sync_error on the Biometric Device Registration record in ERPNext.
        Called after each successful device sync. Replaces the Shift Type update.
        """
        site = self.config.get('FRAPPE_SITE')
        if not site:
            return False
        try:
            url = f"{site.rstrip('/')}/api/method/tb_hotel_core.tb_hotel_core.api.biometric.update_sync_status"
            payload = {
            ...
            if response.status_code == 200:
                print(f"[*] Updated last_sync for device {serial_number} → {timestamp}")
                return True
            else:
                print(f"[!] Failed to update device sync status: HTTP {response.status_code}")
        except Exception as e:
            print(f"[!] Error updating device sync status: {e}")
        return False

    def _get_employee_from_registry(self, sn, user_id):
        """
        Looks up the ERPNext Employee ID for a given ZK device user_id by checking
        the Employee Device Mapping child table stored in device_registry.
        Returns employee name string, or None if not mapped.
        """
        mappings = self.device_registry.get(sn, {}).get("employee_mappings", [])
        user_id_str = str(user_id)
        for m in mappings:
            if str(m.get("attendance_device_id", "")) == user_id_str:
                return m.get("employee")
        return None

    def push_checkin_with_known_employee(self, att, device_sn, device_id, employee_id):
        """
        Creates an Employee Checkin record directly using a known employee ID.
        Used when the registry path resolves the employee — no Employee doctype
        field lookup is performed at all.
        """
        site = self.config.get('FRAPPE_SITE')
        if not site:
            return False, "Config Missing (Site)"
        ...
        try:
            url = f"{site.rstrip('/')}/api/resource/Employee Checkin"
            response = self.session.post(url, json=payload, timeout=10)
            if response.status_code in [200, 201]:
                return True, "Success"
            err_msg = f"HTTP {response.status_code}"
            try:
                server_msgs = response.json().get('_server_messages', '[]')
                if isinstance(server_msgs, str):
                    msgs = json.loads(server_msgs)
                    err_msg = msgs[0].get('message', err_msg) if msgs else err_msg
            except Exception:
                pass
                        ...
                        if "=" in line:
                            ...
                            if key == 'USER_MAP' or key == 'SHIFT_DEVICE_MAP':
                                try:
                                    ...
                                    print(f"[!] WARNING: Failed to parse {key} as JSON: {e}")
                                    print(f"[!] Check config.env for trailing commas or invalid syntax in {key}")
                            else:
                                config[key] = val
        except Exception as e:
    ...
    def get_shifts_for_device(self, device_sn, device_id=None):
        """Returns ALL shift types mapped to a device SN or Device ID, or [default shift type]"""
        ...
        for shift_name, identifiers in mapping.items():
            if device_sn in identifiers or (device_id_str and device_id_str in identifiers):
                matched.append(shift_name)
        if matched:
            return matched
        default = self.config.get('SHIFT_TYPE')
        return [default] if default else []

    def push_to_frappe(self, att, device_sn, device_id):
        ...
        return self.push_to_frappe_fallback(att, device_sn, device_id)

    def push_to_frappe_fallback(self, att, device_sn, device_id):
    ...
    def discover_and_fetch(self, subnets, date_filter):
        ...
        success_count = 0
        shift_max_ts = {}  # {shift_name: max_latest_timestamp} — one entry per shift
        ...
        with ThreadPoolExecutor(max_workers=min(len(found_ips), 5)) as executor:
            ...
            for future in as_completed(future_to_fetch):
                success, latest_ts, dev_shifts = future.result()
                if success:
                    success_count += 1
                    if latest_ts and dev_shifts:
                        for shift in dev_shifts:
                            if shift not in shift_max_ts or latest_ts > shift_max_ts[shift]:
                                shift_max_ts[shift] = latest_ts

        return len(found_ips), success_count, shift_max_ts
    ...
    def fetch_data_from_device(self, ip, date_filter=None, cmd_args=None):
        ...
        try:
            ...
            # 2. Determine actual date filter using ERPNext device registry (primary)
            #    or SHIFT_DEVICE_MAP fallback (legacy).
            actual_filter = date_filter
            is_explicit_date = cmd_args and (cmd_args.today or cmd_args.all or cmd_args.yesterday)
            using_registry = sn in self.device_registry
            device_shifts = []  # Populated only in legacy (non-registry) path

            if not is_explicit_date:
                if using_registry:
                    # PRIMARY PATH — use last_sync from Biometric Device Registration
                    last_sync_str = self.device_registry[sn].get("last_sync")
                    if last_sync_str:
                        try:
                            actual_filter = datetime.strptime(last_sync_str.split('.')[0], "%Y-%m-%d %H:%M:%S")
                            print(f"[*] [{sn}] Using device registry last_sync: {actual_filter}")
                        except ValueError:
                            actual_filter = None
                    else:
                        actual_filter = None  # Never synced — fetch all
                        print(f"[!] [{sn}] Device has never been synced. Fetching ALL records.")
                else:
                    # LEGACY FALLBACK — use earliest last_sync_of_checkin from Shift Types
                    device_shifts = self.get_shifts_for_device(sn, device_id)
                    if device_shifts:
                        earliest_sync = None
                        for shift in device_shifts:
                            ts = self.get_last_sync_from_erpnext(shift)
                            if ts is not None:
                                if earliest_sync is None or ts < earliest_sync:
                                    earliest_sync = ts
                        if earliest_sync is not None:
                            actual_filter = earliest_sync
                            print(f"[*] [{sn}] [legacy] Using earliest last_sync across {len(device_shifts)} shift(s): {earliest_sync}")
                        else:
                            actual_filter = None
                            print(f"[!] [{sn}] [legacy] No last_sync found. Fetching ALL records.")
                    else:
                        device_shifts = []
            ...
            latest_timestamp = None  # Initialize before block so return always has a value
            if attendance:
                # Fetch Device Info
                ...
                for att in attendance:
                    ...
                    if not self.no_push:
                        if using_registry:
                            # PRIMARY: resolve employee from Device Mapping child table only
                            employee_id = self._get_employee_from_registry(sn, att.user_id)
                            if employee_id:
                                success, msg = self.push_checkin_with_known_employee(
                                    att, sn, device_id, employee_id
                                )
                            else:
                                success, msg = False, "Not in mapping table"
                        else:
                            # LEGACY: resolve employee via Employee doctype field
                            success, msg = self.push_to_frappe(att, sn, device_id)
                        if success:
                            push_status = "OK"
            ...
            # Post-push sync update
            if not self.no_push and latest_timestamp:
                if using_registry:
                    # PRIMARY PATH — update Biometric Device Registration record
                    self.update_device_sync_status(sn, latest_timestamp)
                    return True, latest_timestamp, []  # No Shift Type update needed
                # LEGACY PATH — return device_shifts so main() updates Shift Type(s)
            return True, latest_timestamp, device_shifts if not using_registry else []
...
def main():
    ...
    if args.ip:
        ...
        total_found = 1
        success, latest_ts, dev_shifts = fetcher.fetch_data_from_device(args.ip, date_filter, cmd_args=args)
        success_count = 1 if success else 0
        shift_max_ts = {}
        if success and latest_ts and dev_shifts:
            for shift in dev_shifts:
                shift_max_ts[shift] = latest_ts
    else:
        subnets = fetcher.get_local_subnets()
        total_found, success_count, shift_max_ts = fetcher.discover_and_fetch(subnets, date_filter)

    if total_found == 0:
        print("\n[!] No devices discovered. Ensure you are on the same network as the devices.")
        return

    # Update each Shift Type with its own max timestamp — prevents backward movement
    if not no_push and shift_max_ts:
        for shift, ts in shift_max_ts.items():
            fetcher.update_last_sync_in_erpnext(shift, ts)
